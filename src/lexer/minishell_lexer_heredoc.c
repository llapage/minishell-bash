...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   minishell_lexer_heredoc.c                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: llapage <llapage@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/01 12:18:12 by mde-clee          #+#    #+#             */
/*   Updated: 2023/09/30 17:09:32 by llapage          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int *state, t_token **new_token)
{
	if (!(*state & QUOTES))
	{
		...
		{
			*state += NEW_ARGUMENT - HEREDOC + COMPLETED_TOKEN;
			(*input)--;
		}
		...
		t_token **new_token)
{
	...
	if (!(*new_token)->word)
		skip_spaces(input);
	ret = verify_syntax(input, *state, *new_token, HEREDOC_SYNTAX_MASK);
	if (ret)
	{
